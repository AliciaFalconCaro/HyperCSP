...
%The other classes will be separated into two classes:
    ...
    % - UncommonTask class (Group 2, 3) given new label: 0
        ...
        NumberSegments_HandsMovement =1;
        NumberSegments_SecondaryMovement =1;
        ...
        while(Block<=Length) 
            ...
            if (Labels(Block)==1) 
                HandsMovement_start(NumberSegments_HandsMovement)=Block;
                for j=Block:Length
                    if (Labels(j) ~=1)
                        HandsMovement_end(NumberSegments_HandsMovement) = j-1;
                        PreSegmentLabel(i)=1; 
                        data_start(i) =Block;
                        data_end(i) = j-1;
                        NumberSegments_HandsMovement=NumberSegments_HandsMovement+1;
                        Block=j-1;
                        break
                    end
                end
            ...
            elseif (Labels(Block)==2) 
                SecondaryMovement_start(NumberSegments_SecondaryMovement)=Block;
                for j=Block:Length
                    if (Labels(j) ~=2)
                        SecondaryMovement_end(NumberSegments_SecondaryMovement) = j-1;
                        PreSegmentLabel(i)=0;
                        data_start(i) =Block;
                        data_end(i) = j-1;
                        NumberSegments_SecondaryMovement=NumberSegments_SecondaryMovement+1;
                        Block=j-1;
                        break
                    end
                end
